...
async function setIosApplicationVersion(newVersionName, newVersionCode) {
  ...
  if (version) {
    ...
    display(
      chalk.yellow(
        `Will set CFBundleVersion to ${chalk.bold.underline(newVersionCode)}`
      )
    );
    try {
      const plistInfo = plist.parse(fs.readFileSync(paths.infoPlist, "utf8"));
...
async function setAndroidApplicationVersion(newVersionName, newVersionCode) {
  const { version, versionCode } = await getAndroidVersionInfo(
    newVersionName,
    newVersionCode
  );
  ...
  if (versionCode) {
    ...
    display(
      chalk.yellow(
        `Will set Android version to ${chalk.bold.underline(newVersionName)}`
      )
    );
    display(
      chalk.yellow(
      ...
      if (
        ...
        const newAndroidManifest = androidManifest
          ...
          .replace(
            ...
            `android:versionName="${newVersionName}"`
          );
...
const changeVersion = async () => {
  ...
  const appName = JSON.parse(fs.readFileSync(paths.packageJson)).name;
